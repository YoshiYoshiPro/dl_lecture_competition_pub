...
class EventDataAugmentation:
    ...
    def __call__(self, sample):
        ...
        flow_gt = sample['flow_gt'][0] if 'flow_gt' in sample else None
        ...
        event_volume = TF.rotate(event_volume, angle)
        if flow_gt is not None:
                ...
                for key, value in sample.items():
                    if key in keys_to_crop:
                        if isinstance(value, torch.Tensor):
                            sample[key] = T.functional.crop(value, i, j, h, w)
                        elif isinstance(value, list) or isinstance(value, tuple):
                            sample[key] = [T.functional.crop(
